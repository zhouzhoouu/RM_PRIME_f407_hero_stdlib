...
static void gimbal_cali_control(fp32 *yaw, fp32 *pitch, Gimbal_Control_t *gimbal_control_set)
{
    //yaw = 0.0f - gimbal_control_set->gimbal_yaw_motor.relative_angle;
    
    *pitch = 0.0f - gimbal_control_set->gimbal_pitch_motor.inti_pitch_angle/1000.0f;
    *yaw = 0.0f - gimbal_control_set->gimbal_yaw_motor.init_yaw_angle/1000.0f;

    if( fabs(gimbal_control_set->gimbal_yaw_motor.relative_angle) < 0.1f){
        *yaw = 0.0f - gimbal_control_set->gimbal_yaw_motor.relative_angle;
    }
    if( fabs(gimbal_control_set->gimbal_pitch_motor.absolute_angle) < 0.1f){
        *pitch = 0.0f - gimbal_control_set->gimbal_pitch_motor.absolute_angle;
    }
    // if (yaw == NULL || pitch == NULL || gimbal_control_set == NULL)
    // {
    //     return;
