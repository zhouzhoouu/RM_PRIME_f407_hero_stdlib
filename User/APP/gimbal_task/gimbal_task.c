...
static fp32 motor_ecd_to_angle_change_fp32(fp32 ecd, fp32 offset_ecd);
#if GIMBAL_TEST_MODE
//j-scope 帮助pid调参
...
//    save_pos_zero(2,MIT_MODE);
    ...
    while (1)
    {
        ...
        CAN_CMD_Fric(Fric_Set_Current[0],Fric_Set_Current[1], Fric_Set_Current[2], Fric_Set_Current[3]);
				...
				//CAN1 拨弹轮
        CAN_CMD_Stir(0,0, Shoot_Can_Set_Current, 0);
				vTaskDelay(2);

        //CAN2,pitch
        mit_ctrl(1, 0, 0, 0, 0.1, -gimbal_control.gimbal_pitch_motor.given_current,CAN2);
        //CAN1 YAW
        mit_ctrl(2, 0, 0, 0, 0.1, gimbal_control.gimbal_yaw_motor.given_current,CAN1);
        
        // //CAN2,pitch
        // mit_ctrl(1, 0, 0, 0, 0.1f, 0.0f,CAN2);
        // //CAN1 YAW
        // mit_ctrl(2, 0, 0, 0, 0.1f, 0.0f,CAN1);
...
static void GIMBAL_Init(Gimbal_Control_t *gimbal_init)
{
    ...
    //使能pitch轴DM电机
    //CAN2
		enable_motor_mode(1, MIT_MODE,CAN2);
		//使能yaw轴DM电机
    //CAN1
    enable_motor_mode(2, MIT_MODE,CAN1);
    //电机数据指针获取
    ...
    gimbal_init->gimbal_pitch_motor.inti_pitch_angle = gimbal_init->gimbal_pitch_motor.absolute_angle;
    gimbal_init->gimbal_yaw_motor.init_yaw_angle = gimbal_init->gimbal_yaw_motor.relative_angle;
}
...
static void GIMBAL_Feedback_Update(Gimbal_Control_t *gimbal_feedback_update)
{
    ...
    //absolute angle : IMU_DATA
    //relative angle : err between yaw and shoot

    gimbal_control.gimbal_yaw_motor.flag=0;
    //云台数据更新
    ...
    gimbal_feedback_update->gimbal_pitch_motor.relative_angle = gimbal_feedback_update->gimbal_pitch_motor.gimbal_motor_measure->pos - 0.31f;
    ...
    gimbal_feedback_update->gimbal_yaw_motor.relative_angle = motor_ecd_to_angle_change_fp32(gimbal_feedback_update->gimbal_yaw_motor.gimbal_motor_measure->pos,
                                                                                        2.11f);
...
static fp32 motor_ecd_to_angle_change_fp32(fp32 ecd, fp32 offset_ecd)
{
    ...
    return rad_format(relative_ecd);
}
//计算相对角度
static fp32 motor_ecd_to_angle_change(uint16_t ecd, uint16_t offset_ecd)
{
...
static void DM4310_absolute_angle_limit(DM4310_Motor_t *gimbal_motor, fp32 add)
{
    ...
    bias_angle = rad_format(gimbal_motor->absolute_angle_set - gimbal_motor->absolute_angle);
    //云台相对角度+ 误差角度 + 新增角度 如果大于 最大机械角度
    if (gimbal_motor->absolute_angle + bias_angle + add > MAX_PITCH)
    {
        //如果是往最大机械角度控制方向
        if (add > 0.0f)
        {
            //计算出一个最大的添加角度，
            add = MAX_PITCH - gimbal_motor->absolute_angle - bias_angle;
        }
    }
    else if (gimbal_motor->absolute_angle + bias_angle + add < MIN_PITCH)
    {
        if (add < 0.0f)
        {
            add = MIN_PITCH - gimbal_motor->absolute_angle - bias_angle;
        }
    }
...
static void GIMBAL_Control_loop(Gimbal_Control_t *gimbal_control_loop)
{
    ...
    if(switch_is_down(gimbal_control_loop->gimbal_rc_ctrl->rc.s[MODE_CHANNEL])){
        gimbal_control_loop->gimbal_yaw_motor.given_current = 0;
        gimbal_control_loop->gimbal_pitch_motor.given_current = 0;
        return;
    }
    ...
    {
        //enconde角度控制
        if(switch_is_up ( gimbal_control_loop->gimbal_rc_ctrl->rc.s[0]) )
            gimbal_motor_absolute_angle_control(&gimbal_control_loop->gimbal_yaw_motor);
...
void gimbal_offset_init(void)
{
    ...
    gimbal_control.gimbal_yaw_motor.max_relative_angle = PI;
    gimbal_control.gimbal_yaw_motor.min_relative_angle = -PI;
