...
void chassis_power_control(chassis_move_t *chassis_power_control)
{
    ...
    fp32 out = 0.0f;
    uint8_t electric_quantity = 0.0f;
    fp32 POWER_LIMIT = 60.0f;
    get_chassis_power_limit(&POWER_LIMIT);
    {
        get_chassis_power(&chassis_power,&out);
        //printf("CAP:    %.2f,%.2f,%d\n",chassis_power,out,electric_quantity);    
        
        get_chassis_buffer(&chassis_power_buffer);
        //printf("%.2f,%.2f\n",chassis_power_buffer,POWER_LIMIT);
        ...
        if(chassis_power_buffer < WARNING_POWER_BUFF)
        {
            ...
            else
            {
                ...
                //power_scale = 10.0f / WARNING_POWER_BUFF;
            }
            //缩小
