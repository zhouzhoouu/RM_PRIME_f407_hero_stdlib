...
void chassis_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        chassis_set_contorl(&chassis_move);
			  //printf("relative_angle: %f\n",chassis_move.chassis_yaw_motor->relative_angle);
        //底盘控制PID计算
        chassis_control_loop(&chassis_move);
				//发送限制功率给超电
				CAN_CMD_CHASSIS_Power(get_chassis_limit_power(),0,0,0);//get_chassis_limit_power()
		   	//printf("POWER:     %d\n",get_chassis_limit_power());
        //底盘功率限制
...
void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    if(switch_is_down(chassis_move_control_loop->chassis_RC->rc.s[0]))
    {
        chassis_move_control_loop->motor_chassis[0].give_current = 0;
        chassis_move_control_loop->motor_chassis[1].give_current = 0;
        chassis_move_control_loop->motor_chassis[2].give_current = 0;
        chassis_move_control_loop->motor_chassis[3].give_current = 0;
    ...
    //chassis_vector_to_mecanum_wheel_speed(0.0f,0.0f,chassis_move_control_loop->wz_set,wheel_speed);
