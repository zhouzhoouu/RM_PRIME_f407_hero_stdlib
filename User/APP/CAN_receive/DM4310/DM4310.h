...
void enable_motor_mode(uint16_t motor_id, uint16_t mode_id,CAN_TypeDef* CANx);
void disable_motor_mode(uint16_t motor_id, uint16_t mode_id);
void save_pos_zero(uint16_t motor_id, uint16_t mode_id);

void mit_ctrl(uint16_t motor_id, float pos, float vel,float kp, float kd, float torq,CAN_TypeDef* CANx);
